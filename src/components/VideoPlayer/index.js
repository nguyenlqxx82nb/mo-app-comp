...
import { Image, ImageBackground, Platform, StatusBar, StyleSheet, TouchableOpacity, View, ViewPropTypes } from 'react-native';
import Video from 'react-native-video';
import { Color, Constants, CustomIcon, pushModal } from 'mo-app-common';
import FullScreenFullScreenVideoModal from './FullScreenVideoModal';
...
export default class VideoPlayer extends Component {
  ...
  onToggleFullScreen() {
    if (Platform.OS === 'ios') {
      return this.player.presentFullscreenPlayer();
    }

    // Orientation.lockToLandscapeLeft();
    StatusBar.setHidden(true);
    const modal = {
      content: <FullScreenFullScreenVideoModal onClose={this.handleExitFullScreen} video={this.props.video} />
    };
    pushModal(modal);
  }
  ...
  handleOrientation = (orientation) => {
    ...
    //   return this.setState({ fullscreen: true }, () => StatusBar.setHidden(true));
    // }
    // return this.setState({ fullscreen: false }, () => StatusBar.setHidden(false));
  }
  ...
  renderStartButton() {
    ...
    return (
      ...
      >
        <CustomIcon name={'play_video'} size={42} color={Color.background} />
      </TouchableOpacity>
    );
  }
      ...
      <View style={[styles.controls, customStyles.controls]}>
        <TouchableOpacity
          ...
          <CustomIcon
            ...
            name={this.state.isPlaying ? 'Pause' : 'activate'} size={16} />
        </TouchableOpacity>
        {this.renderSeekBar()}
        {this.props.muted ? null : (
          <TouchableOpacity onPress={this.onMutePress} style={customStyles.controlButton}>
            <CustomIcon
              ...
              name={this.state.isMuted ? 'sound-off' : 'sound-on'} size={16} />
          </TouchableOpacity>
        )}
...
VideoPlayer.propTypes = {
  ...
  customStyles: PropTypes.shape({
    ...
    controlIcon: ViewPropTypesVar.style,
    playIcon: ViewPropTypesVar.style,
    ...
    playArrow: ViewPropTypesVar.style,
  }),
